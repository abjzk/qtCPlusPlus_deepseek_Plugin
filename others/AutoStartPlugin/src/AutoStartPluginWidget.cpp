...
void AutoStartWidget::addProgram()
{
    ...
    if (dialog->exec() == QDialog::Accepted)
    {
        ...
        jzk::LFunc::autoRunPath(1,program.name, program.path, program.arguments);
...
void AutoStartWidget::removeProgram()
{
    ...
    jzk::LFunc::autoRunPath(0, _tableWidget->item(row, 0)->text(), _tableWidget->item(row, 1)->text(), _tableWidget->item(row, 2)->text());
    _logger->info(QString(R"({"name":"%1","path":"%2","arguments":"%3"})")
